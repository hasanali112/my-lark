...
const ConversationView = ({ activeUser }: ConversationViewProps) => {
  const messages = [
    { id: 1, sender: 'Sarah', content: 'The latest urban development data just came in for district 4.', time: '9:30 AM', isOwn: false },
    { id: 2, sender: 'Hasan', content: 'Great, let me take a look at the energy consumption charts.', time: '9:31 AM', isOwn: true },
    { id: 3, sender: 'Sarah', content: 'I notice a spike in traffic demand near the new tech park. We might need to adjust the automated routing.', time: '10:15 AM', isOwn: false },
    { id: 4, sender: 'Sarah', content: 'Sending the updated routing parameters now.', time: '10:16 AM', isOwn: false },
    { id: 5, sender: 'Hasan', content: 'Received. Calibrating the sensors now. Will push live in 5 mins.', time: '10:42 AM', isOwn: true },
  ];
        ...
        <div className="flex items-center">
          <div className="relative">
            <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200">
               <Image src={activeUser.avatar} alt={activeUser.name} width={40} height={40} />
            </div>
            <div className={`absolute bottom-0 right-0 w-2.5 h-2.5 border-2 border-white rounded-full ${activeUser.isOnline ? 'bg-green-500' : 'bg-gray-400'}`}></div>
          </div>
          <div className="ml-3">
            <h2 className="text-sm font-bold text-[#1F2329]">{activeUser.name}</h2>
            <p className={`text-[10px] font-medium ${activeUser.isOnline ? 'text-green-600' : 'text-[#646A73]'}`}>
          ...
          <div key={msg.id} className={`flex ${msg.isOwn ? 'justify-end' : 'justify-start'}`}>
            {!msg.isOwn && (
               <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-200 mr-2 mt-1 shrink-0">
                  <Image src="https://i.pravatar.cc/100?img=11" alt={msg.sender} width={32} height={32} />
               </div>
            )}
            <div className={`max-w-[70%] group relative`}>
              ...
              <p className={`text-[10px] text-[#646A73] mt-1 ${msg.isOwn ? 'text-right' : 'text-left'}`}>
                {msg.time}
              </p>
            </div>
          </div>
